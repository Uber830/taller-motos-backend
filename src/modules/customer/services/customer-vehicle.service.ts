...
export class CustomerVehicleService {
  ...
  async createVehicle(
    ...
    if (customerVehicleCount >= 3) throw new BadRequestError("Customer has reached the maximum limit of 3 vehicles");
    ...
    if (existingVehicle) throw new ConflictError("A vehicle with this plate already exists in your workshop");
   ...
   */
  async getVehicleById(vehicleId: string): Promise<VehicleWithCustomer | null> {
  ...
  async updateVehicle(
    ...
    await this.validateVehicleAccess(vehicleId, userId);
  ...
  async deleteVehicle(vehicleId: string, userId: string): Promise<void> {
    ...
    if (vehicleWithWorkOrders && vehicleWithWorkOrders._count.workOrders > 0) {
      throw new BadRequestError("Cannot delete vehicle with associated work orders");
    }
...
export const customerVehicleService = new CustomerVehicleService();
